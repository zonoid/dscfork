...
class StratumElementMedia extends StratumElement
{
	/**
	 * The form field type.
	 *
	...
	protected function getInput( )
	{
		...
		$html[] = '		<a class="modal" title="' . JText::_( 'JLIB_FORM_BUTTON_SELECT' ) . '"' . ' href="' . ($this->readonly ? '' : ($link ? $link : 'index.php?option=com_media&amp;view=images&amp;tmpl=component&amp;asset=' . $asset . '&amp;' . 'fieldid=' . $this->id . '&amp;folder=' . $folder)) . '"' . ' rel="{handler: \'iframe\', size: {x: 800, y: 500}}">';
		...
		if ( $showPreview )
		{
			...
			$attr = array( 'id' => $this->id . '_preview', 'class' => $this->preview_class, 'style' => $this->preview_style );
			...
			if ( $showAsTooltip )
			{
				...
				$options = array( 'title' => JText::_( 'JLIB_FORM_MEDIA_PREVIEW_SELECTED_IMAGE' ), 'text' => JText::_( 'JLIB_FORM_MEDIA_PREVIEW_TIP_TITLE' ), 'class' => 'hasTipPreview ' . $this->preview_tooltip_class );
	...
	public function fetchElement( $name, $value = '', $attribs = array() )
	{
		$this->name = $name;
		$this->value = $value;
		$this->id = isset( $attribs['id'] ) ? $attribs['id'] : (!empty( $this->id ) ? $this->id : $name);
