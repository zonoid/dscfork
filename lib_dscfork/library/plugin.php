...
class StratumPlugin extends JPlugin
{
	...
	protected function _checkToken( $suffix = '', $method = 'post' )
	{
		...
		$r_token = JFactory::getApplication( )->input->$method->get( '$token', '', 'alnum' );

		return $r_token ? true : false;
	}
	...
	{
		$suffix = JFactory::getApplication( )->input->$method->getCmd( 'tokenSuffix' );
	...
	protected function _getMe( )
	{
		if ( empty( $this->_row ) )
		{
			...
			$table = JTable::getInstance( 'Payment', 'SampleTable' );
