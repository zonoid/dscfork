...
class StratumFile extends JObject
{
	...
	function upload( )
	{
		...
		// path
		$dest = $this->getDirectory( ) . DIRECTORY_SEPARATOR . $this->getPhysicalName( );
	...
	function getPhysicalName( $obfuscate = '' )
	{
		...
		if ( $obfuscate )
		{
			...
			while ( $fileexists = JFile::exists( $dir . DIRECTORY_SEPARATOR . $physicalname ) )
			{
				$name = JUtility::getHash( time( ) );
				$physicalname = $name . "." . $extension;
			}
	...
	function textToFile( $file, $temppath = '' )
	{
		...
		$destination = $temppath . DIRECTORY_SEPARATOR . $file->filename;

		if ( $f = @fopen( $destination, 'wb' ) )
		{
			if ( $file->filetext && fwrite( $f, $file->filetext ) )
			{
				$result = $destination;
			}
			fclose( $f );
	...
	function blobToFile( $file, $temppath = '' )
	{
		...
		if ( $f = @fopen( $destination, 'wb' ) )
		{
			if ( $file->fileblob AND fwrite( $f, $file->fileblob ) )
			{
				$result = $destination;
			}
			fclose( $f );
